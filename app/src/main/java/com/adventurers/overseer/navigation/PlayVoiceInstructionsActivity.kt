...
import com.adventurers.overseer.databinding.ActivityNavigationBinding
...
class PlayVoiceInstructionsActivity : AppCompatActivity() {
    ...
    private lateinit var route2: DirectionsRoute
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val sRoute = intent.getStringArrayListExtra("Route");
//        Toast.makeText(this, "sRoute"+sRoute!![0].toString(),Toast.LENGTH_LONG).show()
        val gRoute = ArrayList<Point>()
        val gson = Gson()
        for (r in sRoute!!) {
...
//            updateCamera(Point.fromLngLat(-122.4192, 37.7627))
        ...
        binding.actionButton.setOnClickListener {
            when (mapboxNavigation.getRoutes().isEmpty()) {
                true -> {
                    ...
                    mapboxNavigation.setRoutes(listOf(route2))
                }
                false -> {
                    startSimulation()
    ...
    private fun startSimulation() {
        mapboxReplayer.run {
            ...
            val replayEvents = ReplayRouteMapper().mapDirectionsRouteGeometry(route2)
